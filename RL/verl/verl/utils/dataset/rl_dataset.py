...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        if self.filter_overlong_prompts:
            ...
            self.dataframe = self.dataframe.filter(
                lambda doc: len(tokenizer.tokenize(doc[prompt_key])) <= self.max_prompt_length,
                num_proc=self.num_workers,
                desc=f"Filtering prompts longer than {self.max_prompt_length} tokens"
            )
            # def is_valid_prompt(doc):
            #     try:
            #         text = doc[prompt_key]
            #         print(text)
            #         _ = tokenizer.tokenize(str(text))  # 主动触发 tokenizer 错误
            #         return len(tokenizer.tokenize(str(text))) <= self.max_prompt_length
            #     except Exception as e:
            #         print(f"[Invalid sample] doc = {doc}")
            #         print(f"[Error] {e}")
            #         return False
            #
            # self.dataframe = self.dataframe.filter(
